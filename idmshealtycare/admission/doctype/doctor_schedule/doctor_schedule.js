...
frappe.ui.form.on('Doctor Schedule', {
	refresh: function(frm) {
		cur_frm.fields_dict["doctor_schedule_time_slots"].grid.wrapper.find('.grid-add-row').hide();
		cur_frm.fields_dict["doctor_schedule_time_slots"].grid.add_custom_button(__('Add Time Slots'), () => {
			let d = new frappe.ui.Dialog({
				...
				primary_action: () => {
					...
					if (values) {
						...
						function check_overlap_or_add_slot(week_day, cur_time, end_time, add_slots_to_child){
							...
							if (values.create_slots) {
								while (cur_time < end_time) {
										...
										if (to_time <= end_time) {
											if (frm.doc.doctor_schedule_time_slots){
												frm.doc.doctor_schedule_time_slots.forEach(function(slot) {
											...
											if (add_to_child && add_slots_to_child) {
												frm.add_child('doctor_schedule_time_slots', {
								...
								// appointment for day
								let add_to_child = true;
								if (add_slots_to_child && frm.doc.doctor_schedule_time_slots){
									frm.doc.doctor_schedule_time_slots.forEach(function(slot) {
										if (slot.day == week_day){
											let slot_from_moment = moment(slot.from_time, 'HH:mm:ss');
											let slot_to_moment = moment(slot.to_time, 'HH:mm:ss');
						...
						frm.refresh_field('doctor_schedule_time_slots');
